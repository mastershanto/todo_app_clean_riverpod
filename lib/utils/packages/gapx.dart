...
/// and the path from the [Gapx] widget to its enclosing [Row], [Column], or
...
class Gapx extends StatelessWidget {
  ...
  const Gapx(this.mainAxisExtent, {super.key, this.crossAxisExtent, this.color})
  ...
  ///
  /// The [mainAxisExtent] must not be null and must be positive.
  const Gapx.expand(double mainAxisExtent, {Key? key, Color? color})
    : this(
        mainAxisExtent,
        key: key,
  ...
  Widget build(BuildContext context) {
    ...
    return _RawGap(
      fallbackDirection == Axis.horizontal
          ? mainAxisExtent.w
          : mainAxisExtent.h,
...
/// See also:
///
///  * [Gapx], the unflexible version of this widget.
...
/// A sliver-compatible gap widget that uses ScreenUtil for responsive spacing.
/// Example usage: `SliverGapx(20)`
///
/// It behaves like a sized sliver spacing gap in a CustomScrollView.
class SliverGapx extends StatelessWidget {
  /// The logical space (before scaling) to occupy in the scroll direction.
  final double mainAxisExtent;

  /// Optional background color for visual debugging or design.
  final Color? color;

  const SliverGapx(this.mainAxisExtent, {super.key, this.color})
    : assert(mainAxisExtent >= 0 && mainAxisExtent < double.infinity);
  ...
  Widget build(BuildContext context) {
    ...
    final double sizedExtent = scrollAxis == Axis.vertical
        ? mainAxisExtent.h
        : mainAxisExtent.w;

    return SliverToBoxAdapter(
      child: SizedBox(
        height: scrollAxis == Axis.vertical ? sizedExtent : null,
        width: scrollAxis == Axis.horizontal ? sizedExtent : null,
